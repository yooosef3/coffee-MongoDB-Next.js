...
const list = [
  {
    ...
    title: "لینک های سریع",
    links: [
      { id: 1, name: "درباره شرکت" },
      { id: 2, name: "نظر مشتریان" },
      { id: 3, name: "جوایز" },
  ...
  },
  {
    ...
    title: "موضوعات برتر",
    links: [
      { id: 1, name: "عربیکا سبز" },
      { id: 2, name: "عربیکا برشته" },
      { id: 3, name: "ربوستا" },
      { id: 4, name: "دانه های کنیایی" },
      { id: 5, name: "دانه های مکزیکی" },
    ],
  },
  {
    id: 3,
    ...
    links: [
      { id: 1, name: "سفارش من" },
      { id: 2, name: "راهنمایی" },
      { id: 3, name: "قوانین بازگشت کالا" },
      { id: 4, name: "قوانین و ضوابط" },
      { id: 5, name: "گزارش خطا" },
      { id: 6, name: "بلاگ" },
    ],
  },
...
const Footer = () => {
  ...
  return (
    <footer
      ...
      className="bg-center relative bg-cover bg-no-repeat h-[110vh] lg:h-[75vh]"
    >
      <Image
        src={cup}
        width={200}
        height={170}
        alt="cup"
        className="absolute xl:w-64 xl:top-[-120px] z-10 hidden xl:block"
      />
        ...
        <div className="md:w-[800px] lg:w-[90%] xl:w-[80%] mx-auto">
          <div className="flex flex-col lg:flex-row gap-10 my-10">
            <div className="text-center lg:w-[20%] flex flex-col gap-4 px-4">
              ...
              <p>
                لباس‌های باکیفیت از مواد درجه یک برای ساختن لباسی استفاده
                می‌کنند که مفتخرید آن را متعلق به خودتان بنامید.
              </p>
              ...
              <div>
                <span className="text-teal-500">(5.0) </span>
                <span>براساس 3500 نظر</span>
              </div>
            ...
            <div className="px-4 lg:w-[80%] lg:hidden">
              {list.map((item) => (
                <div key={item.id}>
                  <div
                    onClick={() => setDrop(item.type)}
                    className={`flex justify-between items-center ${
                      drop === item.type && "text-green-500"
                    } cursor-pointer py-2 border-b border-gray-500`}
                  >
                    <h1 className="text-lg font-bold">{item.title}</h1>
                    <BiChevronDown
                      className={`${
                        drop === item.type && "rotate-180 duration-200"
                      }`}
                    />
                  </div>
                  <ul
                    className={`p-3 ${
                      drop === item.type ? "flex flex-col gap-2" : "hidden"
                    }`}
                  >
                    {item.links.map((link) => (
                      <Link
                        href="/"
                        className="hover:text-teal-500 text-gray-300 duration-200 w-fit"
                        key={link.id}
                      >
                        {link.name}
                      </Link>
